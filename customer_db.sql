...
SELECT C.*, CT.CITY FROM CUSTOMERS C JOIN CITIES CT ON CT.ID = C.CITYID WHERE CT.CITY = 'ÝSTANBUL'

-- Customers who live in Istanbul (Subquery)
SELECT (SELECT CITY FROM CITIES WHERE ID = C.CITYID), C.* FROM CUSTOMERS C WHERE (SELECT CITY FROM CITIES WHERE ID = C.CITYID) = 'ÝSTANBUL'

-- Customers who live in Istanbul or Ankara (Subquery)
SELECT * FROM CUSTOMERS C WHERE C.CITYID IN (SELECT ID FROM CITIES WHERE CITY IN ('ÝSTANBUL', 'ANKARA')) 
...
ORDER BY AGEGROUP2
 ...
 SELECT * FROM CUSTOMERS C WHERE CITYID IN (SELECT ID FROM CITIES CT WHERE CITY = 'ÝSTANBUL') AND 
 DISTRICTID NOT IN (SELECT ID FROM DISTRICTS WHERE DISTRICT='KADIKÖY')
